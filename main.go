...
func CryptPSPayload() {
	...
	var psPath string
	gostyle.Write("[@Admin:~/Royaloakap$] > Enter the path to the PowerShell script: ", gostyle.YELLOW_TO_RED, false)
	fmt.Scanln(&psPath)

	psContent, err := ioutil.ReadFile(psPath)
	if err != nil {
		gostyle.Write(fmt.Sprintf("[@Admin:~/Royaloakap$] > Error reading PowerShell script: %s\n", err), gostyle.RED_TO_YELLOW, false)
		return
	}

	layerInput := getinput("[@Admin:~/Royaloakap$] > Enter the number of obfuscation layers: ")
	numLayers, err := strconv.Atoi(layerInput)
	if err != nil {
		gostyle.Write("[@Admin:~/Royaloakap$] > Invalid input. Please enter a valid number.\n", gostyle.RED_TO_YELLOW, false)
		return
	}
	...
	if err != nil {
		gostyle.Write(fmt.Sprintf("[@Admin:~/Royaloakap$] > Error writing obfuscated script: %s\n", err), gostyle.RED_TO_YELLOW, false)
		return
	}

	gostyle.Write(fmt.Sprintf("[@Admin:~/Royaloakap$] > Obfuscated script saved as %s\n", outputPath), gostyle.YELLOW_TO_RED, false)
}
...
Author : Evilbytecode & Codepulze
   ...
   `, gostyle.YELLOW_TO_RED, false)
	...
	gostyle.Write("[@Admin:~/Royaloakap$] >", gostyle.YELLOW_TO_RED, false)
...
func ConvertPSToBAT() {
	reader := bufio.NewReader(os.Stdin)
	gostyle.Write("[@Admin:~/Royaloakap$] > Enter the full path of the PowerShell script file: ", gostyle.YELLOW_TO_RED, false)
	...
	gostyle.Write(fmt.Sprintf("[@Admin:~/Royaloakap$] > Converted: %s\n", opat), gostyle.YELLOW_TO_RED, false)
}
...
func CryptPythonPayload() {
	...
	gostyle.Write("[@Admin:~/Royaloakap$] > Enter the path to the Python script: ", gostyle.YELLOW_TO_RED, false)
	fmt.Scanln(&pythonPath)

	if _, err := os.Stat(pythonPath); os.IsNotExist(err) {
		gostyle.Write("[@Admin:~/Royaloakap$] > Invalid path.\n", gostyle.RED_TO_YELLOW, false)
		return
	}

	pythonContent, err := ioutil.ReadFile(pythonPath)
	if err != nil {
		gostyle.Write(fmt.Sprintf("[@Admin:~/Royaloakap$] > Error reading Python script: %s\n", err), gostyle.RED_TO_YELLOW, false)
		return
	}
	...
	if err := ioutil.WriteFile(outputPath, []byte(obfuscatedContent), 0644); err != nil {
		gostyle.Write(fmt.Sprintf("[@Admin:~/Royaloakap$] > Error writing obfuscated Python script: %s\n", err), gostyle.RED_TO_YELLOW, false)
		return
	}
...
func CryptJSPayload() {
	...
	var jsPath string
	gostyle.Write("[@Admin:~/Royaloakap$] > Enter the path to the JavaScript file: ", gostyle.YELLOW_TO_RED, false)
	fmt.Scanln(&jsPath)
	...
	if err != nil {
		gostyle.Write(fmt.Sprintf("[@Admin:~/Royaloakap$] > Error reading JavaScript file: %s\n", err), gostyle.RED_TO_YELLOW, false)
		return
	}
	...
	if err := ioutil.WriteFile(outputPath, []byte(obfuscatedContent), 0644); err != nil {
		gostyle.Write(fmt.Sprintf("[@Admin:~/Royaloakap$] > Error writing obfuscated JavaScript file: %s\n", err), gostyle.RED_TO_YELLOW, false)
		return
	}
